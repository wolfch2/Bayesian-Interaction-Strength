...
get_coeffs = function(y, log.pred.size, log.prey.size, log.temp, len){
	...
	inits = list(beta=2:5,tau=.3,z=rep(.5,N)) # initial values
	...
	plot(samps,density=FALSE) # check convergence w/ trace plots
	
	return(as.matrix(samps))
}

# use iid multivariate normal likelihood to get mean and covariance matrix parameters for covariates
get_covar_means = function(log.pred.size, log.prey.size, log.temp){
	...
	par(mfrow=c(3,4))
	plot(samps,density=FALSE,auto.layout=FALSE) # check convergence w/ trace plots
	
	samps_mat = as.matrix(samps)
...
bayes_gamma = function(y){
	N = length(y)
	data = list(y=y,N=N)
	inits = list(log.alpha=log(mean(y)^2/var(y)), log.beta=log(mean(y)/var(y)))
	...
	autocorr.plot(samps, lag.max = 10, auto.layout = TRUE, ask = TRUE) # look at correlation in samples
	par(mfrow=c(1,2))
	plot(samps,density=FALSE,auto.layout=FALSE) # check convergence w/ trace plots
...
get_coeffs_all = function(data){
	...
	for(prey in data$prey_types){
		...
		if (length(y) == 1){ # use average if only 1 obs.
			...
			coeffs_mat[prey,,] = get_coeffs(y, log.pred.size, log.prey.size, log.temp, len)
			if (prey == "Austrolittorina cincta") # Since Aus ant. lacks htime obs. -- see unique(htimes_data$htimes$Prey)
				coeffs_mat["Austrolittorina antipodum",,] = get_coeffs(y, log.pred.size, log.prey.size, log.temp, len)
		}
	}
	return(coeffs_mat)
}
...
get_htimes = function(data, coeffs_mat){
	...
	for(prey in data$prey_types){
		...
		out = get_covar_means(log.pred.size, log.prey.size, log.temp) # Sample from mean handling times
		means = out$means
		vars = out$var
...
get_density_means_all = function(data){
	...
	for(prey in data$prey_types){
		...
		N_mat[,prey] = (1-p) * bayes_gamma(y[y>0]) # treat non-zero parts as iid gamma
	}
	return(N_mat)
}
...
bayesian_method = function(data, c=1/3){
	...
	dev.off() # close convergence diag. plot
